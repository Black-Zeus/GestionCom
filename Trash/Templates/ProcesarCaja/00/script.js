...
function displayPendingSales(sales) {
  ...
  tbody.innerHTML = sales.map(sale => {
    ...
    return `
      <tr onclick="selectSale('${sale.folio}')" class="pending-sale-row ${priorityClass} ${expiredClass}" data-folio="${sale.folio}">
        ...
        <td>
          <button class="btn btn-success btn-xs" onclick="event.stopPropagation(); selectSale('${sale.folio}')" ${isExpired ? 'disabled' : ''}>
...
function displaySaleDetails(sale) {
  ...
  document.getElementById('detail-customer-rut').textContent = sale.customer.rut;
  document.getElementById('detail-customer-name').textContent = sale.customer.name;
  document.getElementById('detail-customer-type').textContent = 
    sale.customer.type === 'INDIVIDUAL' ? 'Persona Natural' : 'Empresa';
  document.getElementById('detail-customer-email').textContent = sale.customer.email;

  // Comprador (si aplica)
  const buyerInfo = document.getElementById('buyer-info');
  if (sale.buyer) {
    document.getElementById('detail-buyer-name').textContent = sale.buyer.name;
    document.getElementById('detail-buyer-position').textContent = sale.buyer.position;
  ...
  itemsTbody.innerHTML = sale.items.map(item => `
    <tr>
      ...
      <td class="text-right">$${Math.round(item.unit_price).toLocaleString()}</td>
      <td class="text-center">${item.discount_percentage}%</td>
      <td class="text-right">$${Math.round(item.line_total).toLocaleString()}</td>
    </tr>
  `).join('');

  // Totales
  document.getElementById('detail-total-items').textContent = sale.totals.total_items;
  document.getElementById('detail-subtotal').textContent = `$${Math.round(sale.totals.subtotal).toLocaleString()}`;
  document.getElementById('detail-discounts').textContent = `$${Math.round(sale.totals.line_discounts + (sale.totals.document_discount || 0)).toLocaleString()}`;
  document.getElementById('detail-tax').textContent = `$${Math.round(sale.totals.tax_amount).toLocaleString()}`;
  document.getElementById('detail-total').textContent = `$${Math.round(sale.totals.total_amount).toLocaleString()}`;

  // Información adicional
  document.getElementById('detail-folio').textContent = sale.folio;
  document.getElementById('detail-date').textContent = formatDate(sale.sale_info.date);
  document.getElementById('detail-time').textContent = sale.sale_info.time;
  document.getElementById('detail-seller').textContent = sale.sale_info.seller_name;
  document.getElementById('detail-status').textContent = sale.status.name;
  document.getElementById('detail-credit-type').textContent = sale.credit_sale ? 'Venta a Crédito' : 'Venta Contado';
    ...
    if (savedState) {
      ...
      if (age < maxAge) {
        ...
        if (state.selectedSaleId) {
          const sale = pendingSales.find(s => s.folio === state.selectedSaleId);
          if (sale) {
            selectSale(sale);
          }
        }
