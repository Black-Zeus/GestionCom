...
-- Marcas / modelos / productos / SKU
INSERT IGNORE INTO product_brands (brand_code, brand_name, brand_description, is_active) VALUES
('DEMO_BRD_CECI', 'Ceci Chic Demo', 'Marca propia demo.', TRUE),
('DEMO_BRD_LUNA', 'Luna Demo', 'Marca externa demo.', TRUE);

SET @demo_brand_ceci := (SELECT id FROM product_brands WHERE brand_code = 'DEMO_BRD_CECI');
SET @demo_brand_luna := (SELECT id FROM product_brands WHERE brand_code = 'DEMO_BRD_LUNA');

INSERT IGNORE INTO product_models (brand_id, model_code, model_name, model_description, is_active) VALUES
(@demo_brand_ceci, 'DEMO_MOD_BASIC', 'Basico demo', 'Modelo basico demo.', TRUE),
(@demo_brand_ceci, 'DEMO_MOD_PREM', 'Premium demo', 'Modelo premium demo.', TRUE),
(@demo_brand_luna, 'DEMO_MOD_LUNA', 'Luna urbano demo', 'Modelo urbano demo.', TRUE);
...
SET @demo_unit_par := (SELECT id FROM measurement_units WHERE unit_code = 'DEMO_PAR');
SET @demo_model_basic := (SELECT id FROM product_models WHERE model_code = 'DEMO_MOD_BASIC');
SET @demo_model_prem := (SELECT id FROM product_models WHERE model_code = 'DEMO_MOD_PREM');
SET @demo_model_luna := (SELECT id FROM product_models WHERE model_code = 'DEMO_MOD_LUNA');

INSERT IGNORE INTO products (
  category_id, product_code, product_name, product_description, brand_id, brand,
  product_model_id, model, base_measurement_unit_id, has_variants, is_active,
  has_batch_control, has_expiry_date, has_serial_numbers, has_location_tracking
) VALUES
(@demo_cat_ropa, 'DEMO_PRD_POLERA', 'Polera basica demo', 'Producto demo con variantes de color y talla.', @demo_brand_ceci, 'Ceci Chic Demo', @demo_model_basic, 'Basico demo', @demo_unit_un, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE),
(@demo_cat_ropa, 'DEMO_PRD_BLAZER', 'Blazer premium demo', 'Producto demo de mayor valor.', @demo_brand_ceci, 'Ceci Chic Demo', @demo_model_prem, 'Premium demo', @demo_unit_un, TRUE, TRUE, FALSE, FALSE, FALSE, TRUE),
(@demo_cat_acc, 'DEMO_PRD_CARTERA', 'Cartera urbana demo', 'Producto demo sin serial.', @demo_brand_luna, 'Luna Demo', @demo_model_luna, 'Luna urbano demo', @demo_unit_un, FALSE, TRUE, FALSE, FALSE, FALSE, TRUE);

SET @demo_prd_polera := (SELECT id FROM products WHERE product_code = 'DEMO_PRD_POLERA');
SET @demo_prd_blazer := (SELECT id FROM products WHERE product_code = 'DEMO_PRD_BLAZER');
SET @demo_prd_cartera := (SELECT id FROM products WHERE product_code = 'DEMO_PRD_CARTERA');

INSERT IGNORE INTO product_variants (product_id, variant_sku, variant_name, variant_description, is_default_variant, is_active) VALUES
(@demo_prd_polera, 'DEMO-SKU-POL-NEG-S', 'Polera negra S demo', 'SKU demo color negro talla S.', TRUE, TRUE),
(@demo_prd_polera, 'DEMO-SKU-POL-BEI-M', 'Polera beige M demo', 'SKU demo color beige talla M.', FALSE, TRUE),
(@demo_prd_blazer, 'DEMO-SKU-BLA-NEG-M', 'Blazer negro M demo', 'SKU demo blazer.', TRUE, TRUE),
(@demo_prd_cartera, 'DEMO-SKU-CAR-URB', 'Cartera urbana demo', 'SKU demo cartera.', TRUE, TRUE);

SET @demo_sku_pol_neg := (SELECT id FROM product_variants WHERE variant_sku = 'DEMO-SKU-POL-NEG-S');
SET @demo_sku_pol_bei := (SELECT id FROM product_variants WHERE variant_sku = 'DEMO-SKU-POL-BEI-M');
SET @demo_sku_blazer := (SELECT id FROM product_variants WHERE variant_sku = 'DEMO-SKU-BLA-NEG-M');
SET @demo_sku_cartera := (SELECT id FROM product_variants WHERE variant_sku = 'DEMO-SKU-CAR-URB');

INSERT IGNORE INTO product_barcodes (product_variant_id, barcode_type, barcode_value, measurement_unit_id, is_primary, is_active) VALUES
(@demo_sku_pol_neg, 'EAN13', '7800000001001', @demo_unit_un, TRUE, TRUE),
(@demo_sku_pol_bei, 'EAN13', '7800000001002', @demo_unit_un, TRUE, TRUE),
(@demo_sku_blazer, 'EAN13', '7800000001003', @demo_unit_un, TRUE, TRUE),
(@demo_sku_cartera, 'EAN13', '7800000001004', @demo_unit_un, TRUE, TRUE);

INSERT INTO product_measurement_units (
  product_id, measurement_unit_id, conversion_factor, is_purchase_unit, is_sale_unit, is_inventory_unit, is_active
)
SELECT @demo_prd_polera, @demo_unit_un, 1.000000, TRUE, TRUE, TRUE, TRUE
WHERE NOT EXISTS (SELECT 1 FROM product_measurement_units WHERE product_id = @demo_prd_polera AND measurement_unit_id = @demo_unit_un);

INSERT INTO product_measurement_units (
  product_id, measurement_unit_id, conversion_factor, is_purchase_unit, is_sale_unit, is_inventory_unit, is_active
)
SELECT @demo_prd_cartera, @demo_unit_un, 1.000000, TRUE, TRUE, TRUE, TRUE
WHERE NOT EXISTS (SELECT 1 FROM product_measurement_units WHERE product_id = @demo_prd_cartera AND measurement_unit_id = @demo_unit_un);
