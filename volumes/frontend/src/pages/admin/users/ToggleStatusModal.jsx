...
// =======================
// Componente del Contenido (separado para manejar sus propios estados)
// =======================
const ToggleStatusContent = ({ user, isDeactivating, action, onClose, onConfirm }) => {
  ...
  const getStatusInfo = () => {
    if (isDeactivating) {
      return {
        icon: '🔴',
        color: 'red',
        message: `¿Está seguro que desea desactivar al usuario "${user?.fullName || user?.username}"?`,
        warning: 'El usuario no podrá acceder al sistema hasta que sea reactivado.',
      ...
      return {
        icon: '🟢',
        color: 'green',
        message: `¿Está seguro que desea activar al usuario "${user?.fullName || user?.username}"?`,
      ...
      <div>
        ...
        <textarea
          ...
          className={`block w-full px-3 py-2 border rounded-lg text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 resize-none ${
            error && !reason.trim() ? 'border-red-500' : 'border-gray-300 dark:border-gray-600'
          }`}
