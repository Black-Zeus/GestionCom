...
const customerFields = [
  ...
  { id: 'price_list_group_id', label: 'Tipo de cliente / lista', type: 'select', optionsResource: 'price-list-groups-options', help: 'Define la categoria comercial que se usara por defecto en ventas.' },
...
const customersTabs = [
  {
    ...
    activeValue: 'ACTIVE', forceStatusToggle: true, statusChangeOptions: customerStatusChangeOptions, empty: { customer_type: 'COMPANY', tax_id: '', legal_name: '', commercial_name: '', business_activity: '', country: 'Chile', status_id: '', price_list_group_id: '', default_currency_code: 'CLP', is_credit_customer: false },
    fields: customerFields,
    tableFields: [{ id: 'customer_type', label: 'Tipo', options: customerTypeOptions }, { id: 'tax_id', label: 'RUT' }, { id: 'commercial_name', label: 'Cliente', primary: true, fallbackField: 'legal_name', detailField: 'business_activity' }, { id: 'price_list_group_id', label: 'Lista', optionsResource: 'price-list-groups-options' }, { id: 'contact_person', label: 'Contacto', detailResolver: customerPrimaryContactEmail }, { id: 'email', label: 'Mail' }, { id: 'city', label: 'Ciudad' }, { id: 'default_currency_code', label: 'Moneda', format: currencyDisplay }, { id: 'is_credit_customer', label: 'Credito', format: (value) => (Number(value) === 1 || value === true ? 'Si' : 'No') }],
...
const productBarcodeTabs = [
  {
    id: 'barcodes', resource: 'product-barcodes', label: 'Codigos', singular: 'codigo de barras', icon: Barcode, activeField: 'is_active',    empty: { product_variant_id: '', barcode_type: 'EAN13', barcode_value: '', is_primary: false, is_active: true },
...
const documentTemplateTabs = [
  {
    id: 'templates', resource: 'document-templates', label: 'Plantillas', singular: 'plantilla', icon: FileText, codeField: 'template_code', activeField: 'is_active',    empty: { template_name: '', document_type_id: '', template_channel: 'PDF', paper_size: 'A4', orientation: 'PORTRAIT', is_default: false, is_active: true },
...
const notificationSettingsTabs = [
  {
    id: 'types', resource: 'notification-types', label: 'Tipos', singular: 'tipo de notificacion', icon: Bell, activeField: 'is_active',    empty: { type_code: '', type_name: '', severity: 'INFO', icon_name: 'bell-line', is_user_visible: true, is_active: true },
  ...
  {
    id: 'rules', resource: 'notification-emission-rules', label: 'Reglas', singular: 'regla de emision', icon: Settings, codeField: 'rule_code', activeField: 'is_active',    empty: { rule_name: '', source_label: 'Sistema', notification_type_id: '', min_priority: 1, emit_in_app: true, emit_email: false, emit_push: false, is_active: true },
  ...
  {
    id: 'preferences', resource: 'user-notification-preferences', label: 'Preferencias', singular: 'preferencia', icon: UserRound, activeField: 'is_active',    empty: { user_id: '', notification_type_id: '', receive_in_app: true, receive_email: false, receive_push: false, is_active: true },
...
const cashDenominationsTabs = [
  {
    id: 'denominations', resource: 'currency-denominations', label: 'Denominaciones', singular: 'denominacion', icon: CircleDollarSign, activeField: 'is_active',    empty: { currency_code: 'CLP', denomination_label: '', denomination_type: 'BILL', denomination_value: 0, sort_order: 0, is_active: true },
    fields: [
