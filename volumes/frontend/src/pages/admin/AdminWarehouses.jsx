...
const WarehousePersonnelModal = ({ warehouse, allUsers = [], onClose }) => {
  // serverList = snapshot del servidor (solo se actualiza al cargar/guardar)
  const [serverList, setServerList] = useState([]);
  // draftList = copia editable local
  ...
  const serverIds = useMemo(() => new Set(serverList.map((a) => String(a.user_id))), [serverList]);
  const draftIds = useMemo(() => new Set(draftList.map((a) => String(a.user_id))), [draftList]);
  const serverTypeMap = useMemo(() => new Map(serverList.map((a) => [String(a.user_id), a.access_type])), [serverList]);

  const userOptions = useMemo(
    () => allUsers
      .filter((u) => u.is_active !== false && !draftIds.has(String(u.id)))
      .map((u) => ({ value: String(u.id), label: getUserLabel(u) })),
    [allUsers, draftIds],
  );

  const isDirty = useMemo(() => {
    if (draftList.length !== serverList.length) return true;
    return draftList.some((a) => {
      const orig = serverTypeMap.get(String(a.user_id));
      return orig === undefined || orig !== a.access_type;
    });
  }, [draftList, serverList, serverTypeMap]);

  // Agrega a la lista local sin llamar a la API
  const handleAdd = () => {
    if (!addUserId) return;
    const user = allUsers.find((u) => String(u.id) === String(addUserId));
    if (!user) return;
    setDraftList((prev) => [...prev, {
      user_id: user.id,
      user_full_name: getUserLabel(user),
      user_username: user.username || user.email || '',
      access_type: addAccessType,
      granted_at: null,
      _isNew: true,
    }]);
    setAddUserId('');
  ...
  const handleChangeAccess = (userId, newType) => {
    setDraftList((prev) => prev.map((a) => String(a.user_id) === String(userId) ? { ...a, access_type: newType } : a));
  };

  const handleRevoke = async (access) => {
    // Si es nuevo (no guardado aún), quitar sin confirmar
    if (!access._isNew) {
    ...
    setDraftList((prev) => prev.filter((a) => String(a.user_id) !== String(access.user_id)));
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      for (const item of draftList) {
        if (!serverIds.has(String(item.user_id))) {
          ops.push(warehousesService.grantAccess(warehouse.id, { user_id: Number(item.user_id), access_type: item.access_type }));
        }
      }
      // Revocados
      for (const item of serverList) {
        if (!draftIds.has(String(item.user_id))) {
          ops.push(warehousesService.revokeAccess(warehouse.id, item.user_id));
        }
      }
      // Modificados
      for (const item of draftList) {
        const orig = serverTypeMap.get(String(item.user_id));
        if (orig !== undefined && orig !== item.access_type) {
          ops.push(warehousesService.updateAccess(warehouse.id, item.user_id, { access_type: item.access_type }));
        }
      }
      await notifyPromise(
        Promise.all(ops),
        { loading: 'Guardando cambios...', success: 'Cambios guardados.', error: (e) => getBackendMessage(e, 'No fue posible guardar los cambios.') },
      );
      await loadAccess();
    } catch (err) {
      setError(getBackendMessage(err, 'No fue posible guardar los cambios.'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="flex h-[520px] flex-col gap-4">
      {error && <div className="shrink-0 rounded-md border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-300">{error}</div>}

      {/* Agregar persona — arriba */}
      <div className="shrink-0 rounded-md border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900/60">
      ...
      {/* Tabla personal — abajo con scroll */}
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {draftList.map((access) => {
                const isNew = access._isNew;
                const origType = serverTypeMap.get(String(access.user_id));
                const isModified = !isNew && origType !== undefined && origType !== access.access_type;
                let rowCls = 'bg-white dark:bg-slate-950';
                if (isNew) rowCls = 'bg-emerald-50 dark:bg-emerald-950/20';
                else if (isModified) rowCls = 'bg-amber-50 dark:bg-amber-950/20';
                return (
                  <tr key={access.user_id} className={rowCls}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {isNew && <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">Nuevo</span>}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <select
      ...
      <div className="flex shrink-0 items-center justify-end gap-2 border-t border-slate-200 pt-4 dark:border-slate-800">
  ...
  const warehouseColumns = [
    {
      ...
      render: (warehouse) => (
        <>
          ...
          {(warehouse.location_summary || warehouse.city) && (
            <div className="mt-0.5 flex items-center gap-1 text-xs text-slate-500">
              <MapPin className="h-3 w-3 flex-none" />
              <span>{warehouse.location_summary || warehouse.city}</span>
            </div>
          )}
        </>
      ),
    ...
    },
    {
      ...
      sortable: true,
      align: 'center',
      sortValue: (warehouse) => Number(warehouse.access_count || 0),
      render: (warehouse) => (
        <button
          type="button"
          ...
          onClick={() => openPersonnelModal(warehouse)}
          aria-label={`Ver personal de ${warehouse.warehouse_name}`}
        >
          {Number(warehouse.access_count || 0)}
        </button>
      ),
    },
    {
