...
import { ArrowLeft, CheckCircle2, EyeOff, Layers3, ListChecks, Pencil, RefreshCw, Tags, Trash2, XCircle } from 'lucide-react';
    ...
    <section className="min-h-full bg-slate-50 px-6 py-5 text-slate-950 dark:bg-slate-950 dark:text-white">
      ...
      {activeLevel === 'groups' && (
        <DataTable loading={loading} data={visibleData} footer={pagination} columns={[
          ...
          { id: 'actions', label: 'Acciones', align: 'center', render: (item) => <div className="flex justify-center gap-2"><RowActionButton label="Editar" icon={Pencil} onClick={() => openGroup(item)} /><RowActionButton label="Atributos" icon={Tags} onClick={() => openGroupAttributes(item)} /><RowActionButton label="Eliminar" icon={Trash2} variant="danger" onClick={() => remove('grupo', () => productConfigService.removeGroup(item.id))} /><RowActionButton label={item.is_active ? 'Desactivar' : 'Activar'} icon={item.is_active ? EyeOff : CheckCircle2} variant={item.is_active ? 'danger' : 'neutral'} onClick={() => toggleStatus({ label: 'Grupo', item, payload: { group_name: item.group_name, group_description: item.group_description || null, sort_order: Number(item.sort_order || 0) }, action: (payload) => productConfigService.updateGroup(item.id, payload) })} /></div> },
        ]} />
      )}
      {activeLevel === 'attributes' && (
        <DataTable loading={loading} data={visibleData} footer={pagination} columns={[
