...
import { ArrowLeft, Users, Eye, X } from 'lucide-react';
...
const CHART_ID            = 'customer-sales-bar';
...
const CustomerSales = () => {
  ...
  const fetchData = useCallback(async (nextFilters = filters) => {
    ...
    try {
      ...
      const { data } = await apiClient.get('/reports/sales/customer-sales/data', { params });
      ...
      setTotals(data.totals || { total: 0, txn_count: 0, client_count: 0 });
  ...
  const avgPerClient = rows.length > 0 ? Math.round(totals.total / rows.length) : 0;

  const KPI_ITEMS = [
    { id: 'total',     label: 'Total período',        value: money(totals.total),                     hint: `${fmt(executedFilters.dateFrom)} — ${fmt(executedFilters.dateTo)}` },
    { id: 'clients',   label: 'Clientes activos',     value: rows.length.toString(),                  hint: 'con compras en el período' },
    ...
    { id: 'top',       label: 'Cliente líder',         value: topRow?.customer_name || '—',           hint: topRow ? money(topRow.total) : '' },
    { id: 'avgclient', label: 'Compra prom./cliente',  value: money(avgPerClient),                     hint: 'promedio por cliente activo' },
  ];

  // Chart – horizontal bar, top 10
  const chartData       = rows.slice(0, 10);
  const chartCategories = chartData.map((r) => r.customer_name);
  const chartValues     = chartData.map((r) => Math.round(r.total));

  const chartSeries = [{ name: 'Total por cliente', data: chartValues }];
  ...
  const chartOptions = useMemo(() => ({
    chart: {
      ...
      toolbar: {
        ...
        export: {
          png: { filename: `ventas-cliente_${executedFilters.dateFrom}_${executedFilters.dateTo}` },
          csv: { filename: `ventas-cliente_${executedFilters.dateFrom}_${executedFilters.dateTo}` },
        },
      },
    },
  ...
  const buildExportMetadata = () => [
    ...
    ['Vista',      viewMode === 'detail' ? 'Detalle' : 'Por cliente'],
  ...
  const buildExportData = () => {
    if (viewMode === 'detail') {
      return {
        headers: ['Cliente', 'RUT', 'Fecha', 'Folio', 'Sucursal', 'Método de pago', `Total ${CURRENCY_LABEL}`],
        data: detailRows.map((r) => [
          r.customer_name, r.customer_rut,
          r.sale_date, r.folio, r.warehouse_name, r.payment_method_name, r.total,
        ]),
      };
    }
    return {
      headers: ['Cliente', 'RUT', 'Transacciones', `Total ${CURRENCY_LABEL}`, '% del total', 'Ticket prom.'],
      data: rows.map((r) => [r.customer_name, r.customer_rut, r.txn_count, r.total, r.pct, r.avg_ticket]),
    };
  };
  ...
  const handleXlsxExport = async () => {
    const { headers, data } = buildExportData();
    return buildXlsxBlobUrl(headers, data, 'Ventas por cliente', { metadataRows: buildExportMetadata() });
  };
  ...
  const handlePdfExport = async () => {
    try {
      ...
      const { data: pdfBlob } = await apiClient.post('/reports/sales/customer-sales/pdf', formData, {
  ...
  return (
    <>
      <ReportLayout
        ...
        description={`${fmt(executedFilters.dateFrom)} — ${fmt(executedFilters.dateTo)} · ${rows.length} cliente${rows.length === 1 ? '' : 's'}`}
        actions={[
        ...
        viewModeOptions={[{ id: 'detail', label: 'Detalle' }, { id: 'grouped', label: 'Por cliente' }]}
        ...
        charts={[
          {
            ...
            subtitle: 'Ranking por monto total de ventas en el período',
            icon:     Users,
            content:  rows.length > 0 ? (
              <ReactApexChart
                ...
                key={`customer-bar-${executedFilters.dateFrom}-${executedFilters.dateTo}`}
              />
            ) : (
        ...
        tableTitle={viewMode === 'detail' ? 'Detalle de ventas' : 'Ventas por cliente'}
        ...
        csvFilename={`ventas-cliente_${executedFilters.dateFrom}_${executedFilters.dateTo}.csv`}
        onExportExcel={handleXlsxExport}
        excelFilename={`ventas-cliente_${executedFilters.dateFrom}_${executedFilters.dateTo}.xlsx`}
        onExportPdf={handlePdfExport}
        exportDescription={`${fmt(executedFilters.dateFrom)} — ${fmt(executedFilters.dateTo)}`}
        pdfFilename={`ventas-cliente_${executedFilters.dateFrom}_${executedFilters.dateTo}.pdf`}
        ...
        <div className="overflow-x-auto">
          {loading ? (
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-slate-800/60">
                  {detailRows.map((r, i) => (
                    ...
                    <tr key={`${r.sale_code}-${i}`}>
                      ...
                      <Td muted><span className="text-xs font-mono">{r.customer_rut || '—'}</span></Td>
            ...
            <table className="w-full min-w-[680px]">
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800/60">
                {rows.map((r, i) => (
                  <tr key={r.customer_id ?? r.customer_name} className={i === 0 ? 'bg-violet-50/40 dark:bg-violet-950/10' : ''}>
                    <Td>
                      <div className="flex items-center gap-2">
                        {i === 0 && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-violet-500" title="Cliente líder" />}
                        <span className={i === 0 ? 'font-medium' : ''}>{r.customer_name}</span>
                      </div>
                    </Td>
                    <Td muted><span className="text-xs font-mono">{r.customer_rut || '—'}</span></Td>
                    ...
                    <Td right>
                      <div className="flex items-center justify-end gap-2">
                        <div className="h-1.5 w-16 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800">
                          <div className="h-full rounded-full bg-violet-500" style={{ width: `${Math.min(r.pct, 100)}%` }} />
                        </div>
