...
const CHART_ID = 'petty-cash-category-chart';
...
const PettyCashByCategory = () => {
  ...
  const fetchData = useCallback(async (nextFilters = filters) => {
    ...
    try {
      ...
      const { data } = await apiClient.get('/reports/petty-cash/by-category/data', { params });
      setRows(data.rows || []);
  ...
  const chartSeries = [{ name: 'Total por categoría', data: chartData.map((row) => Math.round(Number(row.total || 0))) }];
  const chartOptions = useMemo(() => ({
    ...
    xaxis: { categories: chartData.map((row) => row.category_name), labels: { formatter: (value) => `$${(value / 1000).toFixed(0)}K`, style: { colors: isDark ? '#94a3b8' : '#64748b', fontSize: '11px' } }, axisBorder: { show: false }, axisTicks: { show: false } },
  ...
  const exportMetadata = () => [
    ['Reporte', 'Caja chica - gastos por categoría'],
    ...
    ['Vista', viewMode === 'detail' ? 'Detalle' : 'Por categoría'],
  ...
  const exportData = () => (viewMode === 'detail' ? {
    ...
    headers: ['Categoría', 'Gastos', `Total ${CURRENCY_LABEL}`, '% total', 'Pendientes', 'Aprobados', 'Rechazados', 'Con comprobante'],
    data: rows.map((row) => [row.category_name, row.count, row.total, row.pct, row.pending, row.approved, row.rejected, row.with_receipt]),
  });
  ...
  const handleXlsxExport = async () => { const { headers, data } = exportData(); return buildXlsxBlobUrl(headers, data, 'Caja chica categoría', { metadataRows: exportMetadata() }); };
  const handlePdfExport = async () => {
    ...
    const { data: blob } = await apiClient.post('/reports/petty-cash/by-category/pdf', formData, { headers: { 'Content-Type': 'multipart/form-data' }, responseType: 'blob' });
  ...
  return (
    <ReportLayout
      ...
      kpiItems={[{ id: 'total', label: 'Total período', value: money(totals.total), hint: 'gastos de caja chica' }, { id: 'count', label: 'Gastos', value: Number(totals.count || 0).toLocaleString('es-CL'), hint: 'registros' }, { id: 'categories', label: 'Categorías', value: Number(rows.length || 0).toLocaleString('es-CL'), hint: 'con movimiento' }]}
      charts={[{ title: 'Top categorías', subtitle: 'Monto acumulado por categoría', icon: BarChart2, content: chartData.length > 0 ? <ReactApexChart options={chartOptions} series={chartSeries} type="bar" height={Math.max(240, Math.min(chartData.length * 38, 420))} /> : <div className="flex h-40 items-center justify-center text-sm text-slate-400">Sin datos para el período seleccionado</div> }]}
      viewMode={viewMode} onViewModeChange={setViewMode} viewModeOptions={[{ id: 'detail', label: 'Detalle' }, { id: 'grouped', label: 'Por categoría' }]}
      tableTitle={viewMode === 'detail' ? 'Detalle de gastos' : 'Resumen por categoría'} tableSubtitle={loading ? 'Cargando...' : `${viewMode === 'detail' ? detailRows.length : rows.length} registros`} tableIcon={ReceiptText}
      onExportCsv={handleCsvExport} onExportExcel={handleXlsxExport} onExportPdf={handlePdfExport}
      csvFilename={`caja-chica-categoria_${executedFilters.dateFrom}_${executedFilters.dateTo}.csv`} excelFilename={`caja-chica-categoria_${executedFilters.dateFrom}_${executedFilters.dateTo}.xlsx`} pdfFilename={`caja-chica-categoria_${executedFilters.dateFrom}_${executedFilters.dateTo}.pdf`}
    >
      <div className="overflow-x-auto">{loading ? <div className="flex items-center justify-center py-16 text-sm text-slate-400">Cargando datos...</div> : (
        <table className="w-full min-w-[860px]"><thead><tr className="border-b border-slate-100 dark:border-slate-800">{viewMode === 'detail' ? <><Th>Fecha</Th><Th>Código</Th><Th>Sucursal</Th><Th>Categoría</Th><Th>Estado</Th><Th right>Monto</Th></> : <><Th>Categoría</Th><Th right>Gastos</Th><Th right>Total</Th><Th right>% total</Th><Th right>Pendientes</Th><Th right>Aprobados</Th><Th right>Rechazados</Th><Th right>Con comprobante</Th></>}</tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800/60">{(viewMode === 'detail' ? detailRows : rows).map((row) => viewMode === 'detail' ? <tr key={row.expense_code}><Td muted>{row.expense_date}</Td><Td>{row.expense_code}</Td><Td muted>{row.warehouse_name}</Td><Td>{row.category_name}</Td><Td muted>{row.expense_status_label}</Td><Td right bold>{money(row.expense_amount)}</Td></tr> : <tr key={row.category_id || row.category_name}><Td>{row.category_name}</Td><Td right muted>{row.count}</Td><Td right bold>{money(row.total)}</Td><Td right muted>{Number(row.pct || 0).toLocaleString('es-CL', { maximumFractionDigits: 1 })}%</Td><Td right muted>{row.pending}</Td><Td right muted>{row.approved}</Td><Td right muted>{row.rejected}</Td><Td right muted>{row.with_receipt}</Td></tr>)}</tbody></table>
      )}</div>
    </ReportLayout>
  );
};

export default PettyCashByCategory;
