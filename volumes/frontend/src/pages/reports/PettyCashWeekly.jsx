...
import { ArrowLeft, BarChart2, ReceiptText, X } from 'lucide-react';
...
const CHART_ID = 'petty-cash-weekly-chart';
...
const PettyCashWeekly = () => {
  ...
  const isFiltered = filters.period !== 'week' || selectedBranchValues.join(',') !== defaultBranchValues.join(',') || filters.categoryId !== ALL_CATEGORY_VALUE || filters.status !== 'all';
  ...
  const fetchData = useCallback(async (nextFilters = filters) => {
    ...
    try {
      ...
      const { data } = await apiClient.get('/reports/petty-cash/weekly/data', { params });
      setRows(data.rows || []);
      setDetailRows(data.detail_rows || []);
      setTotals(data.totals || {});
      setCategoryOptions([{ value: ALL_CATEGORY_VALUE, label: 'Todas las categorías' }, ...(data.category_options || []).map((item) => ({ value: String(item.id), label: item.name }))]);
  ...
  const chartSeries = [{ name: 'Total semanal', data: rows.map((row) => Math.round(Number(row.total || 0))) }];
  const chartOptions = useMemo(() => ({
    ...
    plotOptions: { bar: { borderRadius: 3, columnWidth: rows.length > 10 ? '75%' : '48%' } },
    colors: ['#f59e0b'],
    dataLabels: { enabled: false },
    xaxis: { categories: rows.map((row) => row.week_label), labels: { rotate: rows.length > 6 ? -30 : 0, style: { colors: isDark ? '#94a3b8' : '#64748b', fontSize: '11px' } }, axisBorder: { show: false }, axisTicks: { show: false } },
  ...
  const exportMetadata = () => [
    ['Reporte', 'Caja chica - resumen semanal'], ['Periodo', `${fmt(executedFilters.dateFrom)} - ${fmt(executedFilters.dateTo)}`],
    ['Locación', branchLabel(executedBranchValues)], ['Moneda', CURRENCY_LABEL], ['Vista', viewMode === 'detail' ? 'Detalle' : 'Por semana'], ['Generado el', new Date().toLocaleString('es-CL')],
  ];
  const exportData = () => (viewMode === 'detail' ? {
    headers: ['Fecha', 'Código', 'Sucursal', 'Categoría', 'Estado', `Monto ${CURRENCY_LABEL}`],
    data: detailRows.map((row) => [row.expense_date, row.expense_code, row.warehouse_name, row.category_name, row.expense_status_label, row.expense_amount]),
  } : {
    headers: ['Semana', 'Gastos', `Total ${CURRENCY_LABEL}`, 'Pendientes', 'Aprobados', 'Rechazados', 'Con comprobante'],
    data: rows.map((row) => [row.week_label, row.count, row.total, row.pending, row.approved, row.rejected, row.with_receipt]),
  });
  ...
  const handleXlsxExport = async () => { const { headers, data } = exportData(); return buildXlsxBlobUrl(headers, data, 'Caja chica semanal', { metadataRows: exportMetadata() }); };
  const handlePdfExport = async () => {
    const chartImg = await ApexCharts.exec(CHART_ID, 'dataURI');
    const formData = new FormData();
    formData.append('date_from', executedFilters.dateFrom); formData.append('date_to', executedFilters.dateTo);
    formData.append('warehouse_id', executedBranchValues.length ? executedBranchValues.join(',') : 'all');
    formData.append('category_id', executedFilters.categoryId || ALL_CATEGORY_VALUE); formData.append('status', executedFilters.status || 'all'); formData.append('view_mode', viewMode);
    if (chartImg?.imgURI) formData.append('chart_bar', await (await fetch(chartImg.imgURI)).blob(), 'chart_bar.png');
    const { data: blob } = await apiClient.post('/reports/petty-cash/weekly/pdf', formData, { headers: { 'Content-Type': 'multipart/form-data' }, responseType: 'blob' });
  ...
  return (
    <ReportLayout
      ...
      filterBarTrailing={<><div className="flex items-center gap-2"><span className="shrink-0 text-sm text-slate-500">Categoría</span><div className="w-56"><AutocompleteSelect value={[filters.categoryId]} onChange={([value]) => setFilters((current) => ({ ...current, categoryId: value || ALL_CATEGORY_VALUE }))} options={categoryOptions} clearable={false} buttonClassName="h-10 shadow-none" /></div></div><div className="flex items-center gap-2"><span className="shrink-0 text-sm text-slate-500">Estado</span><div className="w-44"><AutocompleteSelect value={[filters.status]} onChange={([value]) => setFilters((current) => ({ ...current, status: value || 'all' }))} options={STATUS_OPTIONS} clearable={false} buttonClassName="h-10 shadow-none" /></div></div></>}
      onRunReport={() => fetchData(filters)} runReportLoading={loading}
      kpiItems={[{ id: 'total', label: 'Total período', value: money(totals.total), hint: 'gastos de caja chica' }, { id: 'count', label: 'Gastos', value: Number(totals.count || 0).toLocaleString('es-CL'), hint: 'registros' }, { id: 'approved', label: 'Aprobados', value: Number(totals.approved || 0).toLocaleString('es-CL'), hint: `${Number(totals.pending || 0)} pendientes` }]}
      charts={[{ title: 'Gastos por semana', subtitle: 'Total semanal del período ejecutado', icon: BarChart2, content: rows.length > 0 ? <ReactApexChart options={chartOptions} series={chartSeries} type="bar" height={280} /> : <div className="flex h-40 items-center justify-center text-sm text-slate-400">Sin datos para el período seleccionado</div> }]}
      viewMode={viewMode} onViewModeChange={setViewMode} viewModeOptions={[{ id: 'detail', label: 'Detalle' }, { id: 'grouped', label: 'Por semana' }]}
      tableTitle={viewMode === 'detail' ? 'Detalle de gastos' : 'Resumen semanal'} tableSubtitle={loading ? 'Cargando...' : `${viewMode === 'detail' ? detailRows.length : rows.length} registros`} tableIcon={ReceiptText}
      onExportCsv={handleCsvExport} onExportExcel={handleXlsxExport} onExportPdf={handlePdfExport}
      csvFilename={`caja-chica-semanal_${executedFilters.dateFrom}_${executedFilters.dateTo}.csv`} excelFilename={`caja-chica-semanal_${executedFilters.dateFrom}_${executedFilters.dateTo}.xlsx`} pdfFilename={`caja-chica-semanal_${executedFilters.dateFrom}_${executedFilters.dateTo}.pdf`}
    >
      <div className="overflow-x-auto">{loading ? <div className="flex items-center justify-center py-16 text-sm text-slate-400">Cargando datos...</div> : (
        <table className="w-full min-w-[820px]"><thead><tr className="border-b border-slate-100 dark:border-slate-800">{viewMode === 'detail' ? <><Th>Fecha</Th><Th>Código</Th><Th>Sucursal</Th><Th>Categoría</Th><Th>Estado</Th><Th right>Monto</Th></> : <><Th>Semana</Th><Th right>Gastos</Th><Th right>Total</Th><Th right>Pendientes</Th><Th right>Aprobados</Th><Th right>Rechazados</Th><Th right>Con comprobante</Th></>}</tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800/60">{(viewMode === 'detail' ? detailRows : rows).map((row) => viewMode === 'detail' ? <tr key={row.expense_code}><Td muted>{row.expense_date}</Td><Td>{row.expense_code}</Td><Td muted>{row.warehouse_name}</Td><Td>{row.category_name}</Td><Td muted>{row.expense_status_label}</Td><Td right bold>{money(row.expense_amount)}</Td></tr> : <tr key={row.week}><Td>{row.week_label}</Td><Td right muted>{row.count}</Td><Td right bold>{money(row.total)}</Td><Td right muted>{row.pending}</Td><Td right muted>{row.approved}</Td><Td right muted>{row.rejected}</Td><Td right muted>{row.with_receipt}</Td></tr>)}</tbody></table>
      )}</div>
    </ReportLayout>
  );
};

export default PettyCashWeekly;
