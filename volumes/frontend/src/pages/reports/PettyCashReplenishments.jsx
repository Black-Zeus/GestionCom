...
import { ArrowLeft, BarChart2, RefreshCw, X } from 'lucide-react';
...
const CHART_ID = 'petty-cash-replenishments-chart';
...
const PettyCashReplenishments = () => {
  ...
  const fetchData = useCallback(async (nextFilters = filters) => {
    ...
    try {
      ...
      const { data } = await apiClient.get('/reports/petty-cash/replenishments/data', { params });
      setRows(data.rows || []);
  ...
  const chartSeries = [{ name: 'Reposiciones', data: rows.map((row) => Math.round(Number(row.total || 0))) }];
  const chartOptions = useMemo(() => ({
    ...
    plotOptions: { bar: { borderRadius: 3, columnWidth: rows.length > 20 ? '75%' : '48%' } },
    colors: ['#0ea5e9'],
    dataLabels: { enabled: false },
    xaxis: { categories: rows.map((row) => fmt(row.iso)), labels: { rotate: rows.length > 10 ? -45 : 0, style: { colors: isDark ? '#94a3b8' : '#64748b', fontSize: '11px' }, show: rows.length <= 60 }, axisBorder: { show: false }, axisTicks: { show: false } },
  ...
  const exportMetadata = () => [
    ['Reporte', 'Caja chica - historial de reposiciones'],
  ...
  const exportData = () => (viewMode === 'detail' ? {
    headers: ['Fecha', 'Código', 'Sucursal', 'Fondo', 'Autoriza', 'Creado por', `Monto ${CURRENCY_LABEL}`, 'Saldo previo', 'Saldo nuevo', 'Motivo'],
    data: detailRows.map((row) => [row.replenishment_date, row.replenishment_code, row.warehouse_name, row.fund_code, row.authorizer_name, row.created_by_name, row.replenishment_amount, row.previous_balance, row.new_balance, row.replenishment_reason]),
  } : {
    headers: ['Fecha', 'Reposiciones', `Total ${CURRENCY_LABEL}`],
    data: rows.map((row) => [row.iso, row.count, row.total]),
  });
  ...
  const handleXlsxExport = async () => { const { headers, data } = exportData(); return buildXlsxBlobUrl(headers, data, 'Caja chica reposiciones', { metadataRows: exportMetadata() }); };
  const handlePdfExport = async () => {
    ...
    const { data: blob } = await apiClient.post('/reports/petty-cash/replenishments/pdf', formData, { headers: { 'Content-Type': 'multipart/form-data' }, responseType: 'blob' });
  ...
  return (
    <ReportLayout
      ...
      description={`${fmt(executedFilters.dateFrom)} - ${fmt(executedFilters.dateTo)} · ${Number(totals.count || 0).toLocaleString('es-CL')} reposiciones`}
      ...
      kpiItems={[{ id: 'total', label: 'Total repuesto', value: money(totals.total), hint: 'en el período' }, { id: 'count', label: 'Reposiciones', value: Number(totals.count || 0).toLocaleString('es-CL'), hint: 'registros' }, { id: 'avg', label: 'Promedio', value: money(Number(totals.count || 0) ? Number(totals.total || 0) / Number(totals.count || 1) : 0), hint: 'por reposición' }]}
      charts={[{ title: 'Reposiciones por día', subtitle: 'Monto diario del período ejecutado', icon: BarChart2, content: rows.length > 0 ? <ReactApexChart options={chartOptions} series={chartSeries} type="bar" height={280} /> : <div className="flex h-40 items-center justify-center text-sm text-slate-400">Sin datos para el período seleccionado</div> }]}
      viewMode={viewMode} onViewModeChange={setViewMode} viewModeOptions={[{ id: 'detail', label: 'Detalle' }, { id: 'grouped', label: 'Por día' }]}
      tableTitle={viewMode === 'detail' ? 'Detalle de reposiciones' : 'Resumen diario'} tableSubtitle={loading ? 'Cargando...' : `${viewMode === 'detail' ? detailRows.length : rows.length} registros`} tableIcon={RefreshCw}
      onExportCsv={handleCsvExport} onExportExcel={handleXlsxExport} onExportPdf={handlePdfExport}
      csvFilename={`caja-chica-reposiciones_${executedFilters.dateFrom}_${executedFilters.dateTo}.csv`} excelFilename={`caja-chica-reposiciones_${executedFilters.dateFrom}_${executedFilters.dateTo}.xlsx`} pdfFilename={`caja-chica-reposiciones_${executedFilters.dateFrom}_${executedFilters.dateTo}.pdf`}
    >
      <div className="overflow-x-auto">{loading ? <div className="flex items-center justify-center py-16 text-sm text-slate-400">Cargando datos...</div> : (
        <table className="w-full min-w-[900px]"><thead><tr className="border-b border-slate-100 dark:border-slate-800">{viewMode === 'detail' ? <><Th>Fecha</Th><Th>Código</Th><Th>Sucursal</Th><Th>Fondo</Th><Th>Autoriza</Th><Th right>Monto</Th><Th right>Saldo nuevo</Th></> : <><Th>Fecha</Th><Th right>Reposiciones</Th><Th right>Total</Th></>}</tr></thead><tbody className="divide-y divide-slate-50 dark:divide-slate-800/60">{(viewMode === 'detail' ? detailRows : rows).map((row) => viewMode === 'detail' ? <tr key={row.replenishment_code}><Td muted>{row.replenishment_date}</Td><Td>{row.replenishment_code}</Td><Td muted>{row.warehouse_name}</Td><Td>{row.fund_code}</Td><Td muted>{row.authorizer_name}</Td><Td right bold>{money(row.replenishment_amount)}</Td><Td right muted>{money(row.new_balance)}</Td></tr> : <tr key={row.iso}><Td>{row.iso}</Td><Td right muted>{row.count}</Td><Td right bold>{money(row.total)}</Td></tr>)}</tbody></table>
      )}</div>
    </ReportLayout>
  );
};

export default PettyCashReplenishments;
